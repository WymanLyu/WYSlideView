...
//        if (!self.itemPadding) self.itemPadding = DEFAUL_ITEMPADDING;
...
//        [btn setBackgroundColor:[UIColor randomColor]];
        ...
        // 1.3.设置按钮frame
//        CGRect titleSize = [item.title getSize];
    ...
    // 2.排序获取自适应宽度的最大值
//    self.maxWidth = [widthArr getMaxObject];
    ...
    [self setNeedsLayout];
//    [self layoutIfNeeded]; // 此处有错 不能马上跟新 因为第一次时自身仍未有frame
}
...
- (void)layoutSubviews {
    ...
    // 3.逻辑判断
    /*
     1.如果自适应的内容宽度 < 自身宽度，则强制性等比例布局
     2.如果自适应的内容宽度 > 自身宽度，则询问是否采用自动布局
        * 自动布局 则不处理
        * 等比布局 
            + 如果自适应内容的最大宽度 < 自身宽度， 则按自身宽度等比例布局
            + 如果自适应内容的最大宽度 < 自身宽度， 则按内容最大宽度等比例布局
     
     */
    if (self.contentSize.width < self.frame.size.width) { // 内容自适应宽度比设置frame小,只能等比例
        NSLog(@"内容自适应最大宽度比设置可视宽度小,只能等比例布局");
        [self setupSubBtnsScale];
    } else { // 内容自适应最大宽度比设置frame大,根据是否自适应来布局
        // 自适应则不重新布局
        if (self.isSizeToFit) {
            NSLog(@"内容自适应最大宽度比设置可视宽度大,选择了自适应布局");
        } else {
            NSLog(@"内容自适应最大宽度比设置可视宽度大,选择了等比例布局");
            // 不自适应则等比例布局
            [self setupSubBtnsScale];
        }
    }
...
- (void)setupSubBtnsScale {
    ...
    // 1..设置按钮frame
    // 内容自适应最大宽度比设置frame宽度大,则按最大宽度计算，否则则按frame计算
    CGFloat btnW = self.maxWidth * _btnArrM.count > self.frame.size.width ? self.maxWidth : self.frame.size.width / count;
...
//    [self.selectedBtn.titleLabel setFont:[UIFont systemFontOfSize:17.0]];
...
//        self.buttomView.frame.origin.x
